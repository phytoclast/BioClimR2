...
#eco.shp
    #selecteco <- eco.shp[eco.shp$ECO_NAME %in% econame, ]
    selecteco <- eco.shp[eco.shp$id %in% econame, ]
    ...
    selectClim$SP1 <- round(ifelse(selectClim$PPETRatio < 0.5 & selectClim$Surplus < 25 , pmax(selectClim$Surplus/25)  ,1),15)
    ...
    NPP1 <- (MAP-Surplus)*0.8*2
    NPP2 <- min(MAP, PET)*1.2*2
    ...
    metric <- paste("Lat: ",round(Lat,digits=2),"°;  Lon: ", round(Lon,digits=2),"°;  Elev: ",round(Elev,digits=0)," m","\n",
                      "MAAT: ",round(MAAT,digits=1),"°C;  ","MAP: ", round(MAP,0)," mm  NPP: ", round(NPP1,digits=0),"-", round(NPP2,digits=0)," g/m²","\n",
...
#Supplemental Graph1----
 ...
 if(graphxmax - graphxmin > 90 | graphymax - graphymin > 45
 ){
   climmap <-  ggplot() +
